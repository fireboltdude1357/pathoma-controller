...
var PaginatedQueryClient = class {
  constructor(client2, onTransition) {
    this.client = client2;
  ...
  subscribe(name, args, options) {
    ...
    const unsubscribe2 = () => this.removePaginatedQuerySubscriber(token);
...
var ConvexClient = class {
  ...
  onPaginatedUpdate_experimental(query, args, options, callback, onError) {
    ...
    const { paginatedQueryToken, unsubscribe: unsubscribe2 } = this.paginatedClient.subscribe(
      getFunctionName(query),
      args,
...
function subscribeToCommands(onCommand) {
  const convex = getConvexClient();
  const unsubscribe2 = convex.onUpdate(
